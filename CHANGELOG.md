...
- **Form factor & input detection**: `getFormFactor()` (`'phone' | 'tablet' | 'desktop' | 'tv' | 'unknown'`, driven by OS + physical screen size, not touch capability), `detectHasTouch()`, `getPrimaryInput()` (`'mouse' | 'touch' | 'unknown'`, via the `pointer` media feature — live, not cached), `getPixelRatio()`, and `detectIsTV()` (Tizen, webOS, Android TV/Google TV, HbbTV, Fire TV, Sony BRAVIA, VIDAA).
- **Runtime context detection**: `getRuntime()` (`'node' | 'browser' | 'webworker' | 'unknown'`), plus standalone `detectIsNode()`, `detectIsBrowser()`, `detectIsWebWorker()`, `detectIsElectron()` (main or renderer process, with or without Node integration), and `detectIsPWA()` (standalone display mode, iOS home-screen flag, or Trusted Web Activity referrer).
- **React hooks**: `useFormFactor()`, `useRuntime()`, and `usePrimaryInput()` (the only one of the three that updates live, via a `matchMedia` change listener — a hybrid device's keyboard/mouse being attached or detached updates the returned value on its own).
- **Vue composables**: the same three, as readonly refs, from `os-detect/vue`.
